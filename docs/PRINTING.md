...
Download the **[shared lid.stl](../output/lid.stl)** and **one body STL** from the [body examples](../README.md#body-examples--choose-one). Print one body and one lid. The other body downloads are alternative mounting shapes and angles, not extra assembly pieces. All supplied body examples use this same lid.

CAD/STEP files, reference meshes and scripts are only needed for editing/development. If you rebuild with changed dimensions, PCB-fit or fastener settings, use the matching lid generated by that rebuild.
...
1. Print **one chosen body and the shared lid** (or your rebuilt matching pair), then remove supports.
