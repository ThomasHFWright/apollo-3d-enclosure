...
Included on `main`. Compact placement brings the R PRO-1 closer to an inside corner while preserving the existing PCB holders, cover and mounting pads. It remains an opt-in setting; each download retains its own saved configuration.

## Try it in FreeCAD

Open [the compact corridor CAD](../output/compact-corner-poe/apollo-mount.FCStd) using the included generator and macro. It starts with:
...
**Independent wall movement:** edit `LeftWallOffset` or `RightWallOffset` in Parameters → Data, then run the macro. Left/right are viewed facing into the corner. Each value adds that many millimetres of perpendicular distance from its wall while keeping distance from the other wall fixed; the PCB angle and frame stay unchanged. Defaults are 0. These controls also work for standard corner mounts. Flat mounts use RearChamberDepth and ignore these two offsets.

`CompactCorner = No` restores the normal placement rules. It defaults to No and has no effect on flat-wall mounts. All current enclosure downloads already include the complete editable and calculated property set. No initial rebuild is needed to expose the controls. Older personal copies gain missing properties on their next rebuild.

Only [the new body STL](../output/compact-corner-poe/body.stl) and [the existing shared lid](../output/lid.stl) are needed to print. The compact example folder also provides body STEP and assembly STEP files. Rebuilds still write into `output/custom`; preserve any work there first.
...
Only the chamber is trimmed at the lid seating plane. The complete wall-contact frame stays intact, including corners that extend past that plane beside the lid. Solid wedges join the trimmed chamber back to the frame wherever the cut would leave a triangular gap. They taper down to the lid seating edge at 45°, with curved relief around the screw housings, instead of stopping at a fixed width and leaving a step. A 0.2 mm clearance protects the lid and retaining lips; the back stays open. PCB placement is unchanged; the lid, rigid PCB ring and screw bearing pads retain their clearance. The existing corner-tip setback and intentional cable passages still apply.
...
With the redundant rear panels removed, the compact body's solid CAD volume is about 51.2 cm³ versus 55.7 cm³ for the original cable-off print. Actual sliced filament depends on orientation, infill and supports.
...
At yaw −39° and pitch 15°, keeping all four screw mounts behind the lid requires 7.56 mm more forward stand-off than the previous recessed-lid prototype, together with a 7.56 mm sideways shift. The PCB centre is now Z=63.86 mm, still 13.37 mm closer than the standard placement. The complete wall-contact frame is retained and the back stays open. The mirrored +39° case receives the equivalent adjustment.

![Lid above the body at yaw minus 39 and pitch 15](images/compact-yaw39-pitch15.png)

## Checks

Run `python test_compact_corner.py` with the installed FreeCAD libraries. It checks the flat reference against the standard generator, compact ±39° cable-enabled examples, ±39°/15° cases with explicit checks that the chamber stays behind the lid seating plane while the whole wall-contact frame and local closing wedges remain intact, straight/tilted, −70° and ±90° examples, a 110° corner, unchanged lid/PCB contact geometry, CO₂ clearance, material probes in the previously open triangular junction and along the taper to the lid seat, unchanged PCB placement, wall-screw bores and bearing pads, an unobstructed rear opening and valid connected closed meshes. All eight published saved CAD examples are rebuilt from their own parameters into temporary folders, checking both exported parts, bed placement and A1 bounds. Standard flat/corner bodies and lids are also compared geometrically with their saved originals. A further check confirms that adding 5 mm to RearChamberDepth moves the PCB forward exactly 5 mm while preserving connector clearance.

The saved example is generated by executing the actual FreeCAD GUI rebuild macro in an isolated directory, then checking its saved parameters and exports. The project owner reports that the latest printed model worked well. This is a fit/use observation, not a long-term thermal-creep test; existing FEM reports do not validate the changed compact body.

`python test_wall_offsets.py [saved.FCStd]` checks independent wall movement, 90°/110° examples, both corner placement modes, unchanged lid/frame geometry, input validation and both printable exports.
