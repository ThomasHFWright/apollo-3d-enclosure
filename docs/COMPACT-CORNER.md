...
Compact mode separates the shallow screw housings from the enclosure envelope, balances clearance against both walls, and lets the chamber extend into the hollow corner. The mounting pads and screw centres stay in place. The PCB, lid and retention rails remain rigid. This is the minimum of the conservative placement envelopes plus the requested extra stand-off, not proof of the absolute smallest possible enclosure.
...
The compact body's solid CAD volume is about 61.4 cm³ versus 55.7 cm³ for the original cable-off print. It uses the corner space more fully and is not a filament-saving change. Actual sliced filament depends on orientation, infill and supports.
...
Run `python test_compact_corner.py` with the installed FreeCAD libraries. It checks the source corridor against the standard generator, compact ±39° cable-enabled examples, straight/tilted and −70° examples, a 110° corner, unchanged lid/PCB contact geometry, CO₂ clearance, wall-screw bores and valid connected closed meshes. Flat-wall behaviour is compared with the original saved preset. A further check confirms that adding 5 mm to RearChamberDepth moves the PCB forward exactly 5 mm while preserving connector clearance.
