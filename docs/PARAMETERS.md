...
Positive pitch tilts down. Positive yaw rotates front-facing +Z toward +X. A corner mount at zero yaw faces its corner bisector. Viewing from behind reverses apparent left/right. The CompactCorner option changes the placement rule and chamber-depth datum as described below. Rotating the PCB can increase stand-off to avoid walls; it also shifts the PCB sideways. Parameters interact, so listed ranges alone do not guarantee a valid combination.
...
| `CompactCorner` | No | Corner mounts only. Yes balances the rigid holder/cover and rear component envelope between the two walls, allowing the chamber to taper into the hollow corner. Reserves the four wall-screw bearing pads when positioning the PCB. Only the chamber is trimmed at the lid seating plane; wall-contact frame corners remain intact, even where they extend past that plane beside the lid. Tapered wedges close gaps between the chamber and frame and meet the lid seating edge with 0.2 mm clearance for the lid and retaining lips, without moving the PCB. RearChamberDepth becomes extra forward stand-off from this calculated placement. Enable CableEnabled to include the provisional plug; cable-off does not reserve a plug or bend. Inspect the complete route and wall-entry needs. |
