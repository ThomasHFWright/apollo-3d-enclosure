...
| `CompactCorner` | No | Experimental, corner mounts only. Yes balances the rigid holder/cover and rear component envelope between the two walls, allowing the chamber to taper into the hollow corner. RearChamberDepth becomes extra forward stand-off from this calculated placement. Enable CableEnabled to include the provisional plug; cable-off does not reserve a plug or bend. Inspect the complete route and wall-entry needs. |
