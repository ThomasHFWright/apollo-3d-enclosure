...
| `CompactCorner` | No | Experimental, corner mounts only. Yes balances the rigid holder/cover and rear component envelope between the two walls, allowing the chamber to taper into the hollow corner. Keeps the body beneath the lid by reserving the four wall-screw bearing pads and trimming unused frame corners; this can require extra stand-off and sideways movement. RearChamberDepth becomes extra forward stand-off from this calculated placement. Enable CableEnabled to include the provisional plug; cable-off does not reserve a plug or bend. Inspect the complete route and wall-entry needs. |
